...
{ 	
	...
	readWriteTiming.FSMC_DataSetupTime = 0x0f; 	        //数据保存时间为60个HCLK	=6*60=360ns
	...
	writeTiming.FSMC_DataSetupTime = 0x03;    	 	    //数据保存时间为6ns*9个HCLK=54ns
	...
	FSMC_NORSRAMInitStructure.FSMC_BurstAccessMode =FSMC_BurstAccessMode_Disable;   //突发式访问失能
	FSMC_NORSRAMInitStructure.FSMC_WaitSignalPolarity = FSMC_WaitSignalPolarity_Low;//突发模式下有效
	FSMC_NORSRAMInitStructure.FSMC_AsynchronousWait=FSMC_AsynchronousWait_Disable;  //在传输期间禁用等待信号（仅仅异步有效）
	FSMC_NORSRAMInitStructure.FSMC_WrapMode = FSMC_WrapMode_Disable;                //禁用FLASH的Wrapped突发访问模式
	...
	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID                   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ID  2017-4-21
